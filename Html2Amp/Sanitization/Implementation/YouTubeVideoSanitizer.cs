...
namespace Html2Amp.Sanitization.Implementation
{
	public class YouTubeVideoSanitizer : MediaSanitizer, IScriptsDependable
	{
		...
		public override IElement Sanitize(IDocument document, IElement htmlElement)
		{
			...
			var srcAttributeValue = ResolveSrcAttributeValue(htmlElement);
			Uri videoUri = new Uri(srcAttributeValue);
		...
		private static string ResolveSrcAttributeValue(IElement htmlElement)
		{
			var srcAttributeValue = htmlElement.GetAttribute(AttributeNames.Src);
			if (srcAttributeValue != null && srcAttributeValue.StartsWith("//"))
			{
				srcAttributeValue = srcAttributeValue.Insert(0, "http:");
			}
